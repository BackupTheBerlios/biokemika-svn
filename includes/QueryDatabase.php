...
// Anmeldung in MetaSearch_body.php

/**
 * @class MsQueryDatabase: Represents an searchable Database
 * that can execute MsQuery objects. This class has to be
 * extended by all database implementions.
 **/
abstract class MsQueryDatabaseDriver extends MsDriver {
 ...
 **/
class MsQuery {
	#public $keyword; => moves to
	public $page_controller;
	public $database;

	function __construct(MsQueryPage $page, MsDatabase $database) {
		$this->page = $page;
		$this->database = $database;
	}

	/// create array from array, for lack of a good PHP map() implemention,
	/// like perls ;-)
	static function create_for_databases(MsQueryPage $page, array $databases) {
		$queries = array();
		foreach($databases as $db) {
			$queries[] = new MsQuery($page, $db);
		}
		return $queries;
	}
